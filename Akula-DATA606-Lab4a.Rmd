...
**_A: Above histogram suggests it is unimodal. It also suggests that population distribution is right skewed._**
...
**_A: Above histogram suggests it is unimodal. It also suggests that sample distribution is right skewed and matches distribution of the population._**
...
**_A:_**
**_1. Histogram generated from the second sample suggests it is unimodal. It also suggests that sample-2 distribution is also right skewed and matches with the distribution of sample-1. Mean of Sample-2 is slightly higher than Sample-1 and actual population._**

**_2. Histogram generated from the third sample of size 100 suggests it is unimodal. It also suggests that sample-3 distribution is also right skewed and matches with the distribution of the actual population. Mean of Sample-3 is slightly higher than actual population._**

**_3. Histogram generated from the fourth sample of size 1000 suggests it is unimodal. It also suggests that sample-4 distribution is also right skewed and matches with the distribution of the actual population. Mean of Sample-4 is slightly higher than actual population._**

**_4. Looking at the samples of size 50, mean of sample-1 is higher than sample-2. But they both are lower than the actual population's mean, as individual observations are influencing the mean of each smaller sample. On the other hand, as the sample size is increasing mean of the sample is moving towards actual population mean. It is fair to conclude as sample size increases mean gets closer to the actual population mean._**
...
**_A: Vector `sample_mean50` has 5000 elements, sample distribution suggests it is unimodal and is not strongly skewed. The distribution of sample means closely resembles the normal distribution. The majority of the population is within one standard deviation. The histogram may not change much with 50,000 sample means. It would provide a more accurate estimate of the population mean._**
...
**_A: Vector `sample_means_small` has 100 elements. Each element represents mean of sample observations._**

```{r , echo=T}
# Initialize the vector
sample_means_small <- rep(NA, 100)

# Loop for 100 times and store the value of the sample mean
for(i in 1:100){
   samp <- sample(area, 50)
...
**_A: When sample size increases, means get closer to the actual population mean. The distribution of sample means closely resembles the normal distribution. The population within one standard deviation is increasing, and skewness of the distribution is disappearing._**
...
**_A: The best point of estimate for the price from samples of size 50 is 181649._**

```{r , echo=T}
# Initialize the vector
sample_means_price50 <- rep(NA, 50)

# Loop for 50 times and store the value of the sample mean
for(i in 1:50){
   samp <- sample(price, 50)
   sample_means_price50[i] <- mean(samp)
}

# Output of 50 means for sample size of 50
sample_means_price50
mean(sample_means_price50)

# Mean of actual population
mean(price)
...
**_A: The sampling distribution shows histogram is unimodal and approximately symmetric. Based on this sampling distribution for a sample of size 50, mean home price in Ames is $\bar{x}_{price}$ = 180973.50. The sampling distribution mean falls within one standard deviation of actual population mean 180796.10._**

```{r, echo=T, eval=TRUE}
# Initialize the vector
sample_means50 <- rep(NA, 5000)

# Loop for 5000 times and store the value of the sample mean
for(i in 1:5000){
   samp <- sample(price, 50)
   sample_means50[i] <- mean(samp)
}
...
**_A: The sampling distribution shows histogram is unimodal and approximately symmetric. Based on this sampling distribution for a sample size of 150, mean home price in Ames is  $\bar{x}_{price}$ = 180754.10. The sampling distribution mean falls within one standard deviation of actual population mean 180796.10. After increasing sampling size, it is observed mean home price moved closer to actual population mean._**
...
**_A: Out of two random samples, one of size 50 and another of size 150, larger sample tends to provide a more accurate estimate. Hence large spread is preferable over the small spread._**
